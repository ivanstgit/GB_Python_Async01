...
class PasswordHash():
    ...
    def generate_password_hash(login: str, password: str) -> str:
        ...
        passwd_hash = hashlib.pbkdf2_hmac(hash_name=PasswordHash._password_hash_name,
...
class TranportEncryption():
    ...
    def check_if_encrypted(msg: bytes) -> bool:
        if (len(msg) >= TranportEncryption.msg_prefix_len
                and msg[:TranportEncryption.msg_prefix_len] == TranportEncryption.msg_prefix):
            return True
        return False
    ...
    def encrypt_message(self, message: bytes) -> bytes:
        if self._session_key:
            try:
                ...
                ciphertext, tag = cipher_aes.encrypt_and_digest(self._prepare_to_encrypt(message))
                res = cipher_aes.nonce + tag + ciphertext
                return self.msg_prefix + res
            except Exception:
                raise UtilsSecurityValidationError
        raise UtilsSecurityNoSessionKeyError
...
class ClientSecurity(TranportEncryption):
    ...
    def __init__(self, key_file_name: str, secret_code: str) -> None:
        ...
        if os.path.exists(key_file_name):
            with open(key_file_name, mode='rb') as f:
                encoded_key = f.read()
        ...
        if not key:
            ...
            encoded_key = key.export_key(passphrase=secret_code, pkcs=8,
                                         protection="scryptAndAES128-CBC")
            with open(key_file_name, mode='wb') as f:
                f.write(encoded_key)
        self._key = key
    ...
    def process_auth_step3(self, message_step2: str, login: str, password: str) -> str:
        ...
        self._server_salt = msg[self._session_key_length:self._session_key_length + self._salt_length]
        passwd_hash = PasswordHash.generate_password_hash(login, password).encode()
        salt = self._server_salt

        hash = hmac.new(passwd_hash, salt, self._digest_hash_name)
        digest = hash.digest()
